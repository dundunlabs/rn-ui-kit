import type { StyleProp, ImageProps, ImageStyle, TextStyle, ViewStyle, TextProps } from "react-native"
import type { ComponentStyles } from "../../types"
import type { Theme } from ".."

export type AvatarVariant = 'circular' | 'rounded' | 'square'
export type AvatarSize = 'sm' | 'md' | 'lg'
export interface AvatarStyles {
  variants: Record<AvatarVariant, ImageStyle | null>
  sizes: Record<AvatarSize, ImageStyle | null>
}
export interface AvatarProps extends Omit<Partial<ImageProps>, 'style'> {
  variant?: AvatarVariant
  size?: AvatarSize | number
  renderText?: (text: string) => React.ReactNode
  style?: StyleProp<ViewStyle> | undefined
  imageStyle?: StyleProp<ImageStyle> | undefined
  textStyle?: StyleProp<TextStyle> | undefined
  placeholder?: string
}
export type AvatarTextProps = TextProps & Pick<AvatarProps, 'size'>
export interface AvatarTextStyles {
  sizes: Record<AvatarSize, TextStyle | null>
}

export const defaultAvatarStyles: ComponentStyles<Theme, AvatarProps, AvatarStyles> = {
  variants: {
    circular: {
      borderRadius: 9999
    },
    rounded: {
      borderRadius: 4
    },
    square: null
  },
  sizes: {
    sm: {
      width: 32,
      height: 32
    },
    md: {
      width: 40,
      height: 40
    },
    lg: {
      width: 48,
      height: 48
    }
  }
}

export const defaultAvatarProps: AvatarProps = {
  variant: 'circular',
  size: 'md',
  renderText: (text: string) => text[0],
  placeholder: 'data:image/png;base64,'
}

export const defaultAvatarTextStyles: ComponentStyles<Theme, AvatarTextProps, AvatarTextStyles> = {
  sizes: {
    sm: {
      fontSize: 16
    },
    md: {
      fontSize: 20
    },
    lg: {
      fontSize: 24
    }
  }
}